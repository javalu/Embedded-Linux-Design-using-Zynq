...
int main(int argc, char *argv[])
{
	...
	printf("GPIO access through /dev/mem, page size: %d\n", page_size);
	...
	/* Read value from the device register */
		*((unsigned *)(ptr + page_offset + GPIO_TRI)) = 0xf;
		value = *((unsigned *)(ptr + page_offset + GPIO_DATA));
		printf("base address: 0x%08x input: %08x\n", gpio_addr, value);
	} else {
	/* Write value to the device register */
		*((unsigned *)(ptr + page_offset + GPIO_TRI)) = 0x0;
		*((unsigned *)(ptr + page_offset + GPIO_DATA)) = value;
		printf("base address: 0x%08x output: %0x\n", gpio_addr, value);
	}
